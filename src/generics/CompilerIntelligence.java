...
// Apple к Fruit, компилятор просто отказывается вызывать метод (такой, как add()), если в списке аргументов
// присутствует маска.
...
// ClassCastException при (Orange)fruit.get(). Метод set() не будет работать с Apple или Fruit, потому что аргумент
// set() тоже объявлен как «? Extends Fruit»; это означает, что им может быть что угодно, а компилятор не может
// проверить безопасность типов для «чего угодно».
class Holder<T> {
    private T value;
    public Holder() {}
