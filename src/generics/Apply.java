...
// Обобщим идею FilledList и создадим параметризованный метод fill(), который получает последовательность и заполняет ее
// при помощи Generator. Попытка запрограммировать это решение на Java сталкивается с проблемой: не существует удобного
// интерфейса Addable (сходного с интерфейсом Iterable из предыдущего примера). Итак, вместо формулировки «любой объект,
// для которого можно вызвать add()» необходимо использовать формулировку «подтип Collection». Полученный код не является
// полностью обобщенным, потому что он должен ограничиваться для работы с реализациями Collection. При попытке использования
// класса, не реализующего Collection, наш обобщенный код работать не будет.
