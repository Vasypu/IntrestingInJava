 ...
 *  <p>
 *  Для достижения своей цели метод hashCode() должен быть быстрым и содержательным, то есть он обязан возвращать
 *  хеш-код, вычисленный на основании внутреннего состояния объекта. Помните, что от этого значения не следует
 *  ждать обязательной уникальности — лучше склоняться к скорости, чем к уникальности, — но объект должен однозначно
 *  идентифицироваться комбинацией hashCode() и equals().
 *  <p>
 *  В методе main() создается несколько объектов CountedString с одинаковыми строками, чтобы показать, что на
 *  самом деле возникают уникальные объекты (для этого и создана переменная id). Также показана таблица HashMap,
 *  чтобы вы увидели, как хранятся внутри нее данные (без видимого порядка), а поиск каждого ключа проводится
 *  индивидуально, чтобы вы убедились в работоспособности механизма поиска.
 */
